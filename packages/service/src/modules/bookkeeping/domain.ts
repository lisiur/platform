...
/**
 * Default pockets flagged via meta so the quick-entry dialog can prefill the
 * asset-side fields: `defaultDebit` fills the debit side ("Received Into" for
 * income, destination for transfers), `defaultCredit` fills the credit side
 * ("Paid From" for expenses, origin for transfers).
 */
const EN: StarterAccount[] = [
  {
    name: "Default Receiving Account",
    type: "asset",
    sortOrder: 5,
    icon: "📥",
    meta: { defaultDebit: true },
  },
  {
    name: "Default Paying Account",
    type: "asset",
    sortOrder: 6,
    icon: "📤",
    meta: { defaultCredit: true },
  },
  ...
  { name: "Opening Balance", type: "equity", sortOrder: 60, icon: "🧮" },
...
const ZH: StarterAccount[] = [
  ...
  { name: "期初余额", type: "equity", sortOrder: 60, icon: "🧮" },

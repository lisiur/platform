...
interface ApplicationAiAgentUiFormProps {
  appId: string;
}

/**
 * Visual AI Agent config — which chat UI parts the end user sees. Independent
 * from the functional settings in {@link ApplicationAiAgentForm}. Fields are
 * seeded under the `ai-agent-ui` group (currently two booleans) and rendered
 * via the shared {@link ConfigField} (checkboxes).
 */
export function ApplicationAiAgentUiForm({
  appId,
}: ApplicationAiAgentUiFormProps) {
    ...
    async function load() {
      ...
      try {
        const res = await withApiFeedback(
          ...
          param: { id: appId, group: "ai-agent-ui" },
        });
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>{ta("aiAgentUiTitle")}</CardTitle>
      </CardHeader>
      <CardContent>

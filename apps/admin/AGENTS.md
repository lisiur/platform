...
- `src/lib/api/app-client.ts` builds the client via `createAppClient<typeof app>("admin", manifest)` (from `@repo/frontend`), deriving the API origin from `manifest.json`'s gateway port; keep the app-code header unless the backend contract changes.

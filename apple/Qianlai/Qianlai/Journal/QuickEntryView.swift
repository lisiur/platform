...
struct QuickEntryView: View {
    ...
    /// Compact icon+name capsule for the recents row — the shared
    /// category badge at the 20pt legend size, so the icon (disc, parent
    /// corner badge) reads like every other surface.
        ...
        return Button {
            ...
            HStack(spacing: 6) {
                CategoryIconBadge(
                    ...
                    fallbackSymbol: "tag",
                    // The account tree carries only parentId — resolve the
                    // parent's glyph from the tree itself.
                    parentIcon: entry.account.parentId.flatMap { parentId in
                        categoryTree.first { $0.account.id == parentId }?
                            .account.icon
                    },
                    diameter: 20
                )

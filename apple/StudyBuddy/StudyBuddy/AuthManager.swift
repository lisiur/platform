...
final class AuthManager {
    ...
    var quickLoginEmail: String? {
        guard keychain.read(account: Self.passwordAccount) != nil else { return nil }
    ...
    func restoreSession() async {
        ...
        guard
            let (data, response) = try? await client.send("GET", "auth/get-session"),
    ...
    func loginWithBiometrics() async throws {
        ...
        try await context.evaluatePolicy(
            ...
            localizedReason: "Unlock your StudyBuddy account."
        )

        guard
            let data = keychain.read(account: Self.passwordAccount),
    ...
    private func storeQuickLogin(email: String, password: String) {
        ...
        keychain.write(Data(password.utf8), account: Self.passwordAccount)
    }
}
...
enum QuickLoginError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .noStoredCredentials: "No saved account. Sign in with your password first."
        case .biometricsUnavailable: "Biometrics are not set up on this device."
        }
    }
}
